...
const News = () => {
    ...
    const articles = articleCache[currentPageArticle] || [];
    ...
    return (
        <div className="w-full px-[10%] pt-8">
            <div className="px-[35px] pb-[30px] bg-white">
                {articles.length > 0 ? (
                    articles.map(article => (
                        <div key={article._id} className="w-full py-[35px] border-b border-b-[#eee] flex justify-between gap-[50px]">
                            <NavLink to={`/news/detail/${article.slug}`} className="max-w-[200px] inline-block">
                                <img src={article.thumbnail} alt={article.subject} className="w-full" />
                            </NavLink>
                            <div className="flex-1 flex flex-col text-left">
                                <NavLink to={`/news/detail/${article.slug}`} className="text-[20px] font-[500] text-[#393939] line-clamp-1">
                                    {article.subject}
                                </NavLink>
                                <div className="block text-[14px] text-[#ccc] my-2">by {article.author} on {article.dateFormat}</div>
                                <div className="text-[15px] line-clamp-2 leading-6">{article.content}</div>
                ...
                <PageBar currentPage={currentPageArticle} totalPage={totalPageArticle} />
            </div>
        </div>
    )
}
